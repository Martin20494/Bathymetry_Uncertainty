...
def polygon_generation(
    ...
    polygon_dataframe.to_file(
        fr"{bathy_shapefile}\\{output_name}\\bathy_{number_simulation}\\polygon_{number_simulation}.shp",
        crs=2193
    )
...
def value_sea_change(
    ...
    if polygon:
        ...
        shapefile_sea_dir = fr"{bathy_shapefile}\\sea\\bathy_{number_simulation}"
        ...
        # Write out file
        dem_xr_domain_copy.rio.to_raster(
            fr"{dem_tiff_path}\\generated_dem_bathy_{number_simulation}.tif", cache=False)
        startdepth_copy.rio.to_raster(fr"{startdepth_tiff_path}\\startdepth_{number_simulation}.tif",
                                      cache=False)
        n_copy.rio.to_raster(fr"{n_tiff_path}\\generated_n_bathy_{number_simulation}.tif",
                             cache=False)
